...
test_func <- function(member_df, end_time, rpt_dur=7){
    ...
    fstd_df <- get_member_growth(member_df, start_time = start_time, end_time = end_time, m_source = "07") %>%
                select(mydate,
                        fstd_member = member_count, 
                        fstd_cum = cum_sum)
    
    cqtd_df <- get_member_growth(member_df, start_time = start_time, end_time = end_time, m_source = "05") %>%
              select(mydate,
                        cqtd_member = member_count, 
                        cqtd_cum = cum_sum)
    ...
    output <- hq_df %>% 
                    ...
                    inner_join(fstd_df, by = 'mydate') %>% 
                    inner_join(cqtd_df, by = 'mydate') 
    return(output)
